        ...
        if (ws_layout[current_workspace] == MONOCLE) {
            ...
            if (n == 1) {
                XMoveResizeWindow(display, tiling_clients[0]->window, gappx, bar_h + gappx, 
                                  sw - 2*gappx - 2*border_width, sh - bar_h - 2*gappx - 2*border_width);
            } else {
                int mw = static_cast<int>(sw * ws_mfact[current_workspace]);
                XMoveResizeWindow(display, tiling_clients[0]->window, gappx, bar_h + gappx,
                                  mw - gappx - (gappx/2) - 2*border_width, sh - bar_h - 2*gappx - 2*border_width);
                
                int th = (sh - bar_h - 2*gappx) / (n - 1);
                for (int i = 1; i < n; i++) {
                    XMoveResizeWindow(display, tiling_clients[i]->window, mw + (gappx/2), bar_h + gappx + (i-1)*th,
                                      sw - mw - gappx - (gappx/2) - 2*border_width, th - gappx - 2*border_width);
                }
            }
            ...
            for (int i = 0; i < n; i++) {
                ...
                XMoveResizeWindow(display, tiling_clients[i]->window, 
                                  ...
                                  cw - 2*gappx - 2*border_width, rh - 2*gappx - 2*border_width);
            }
        }
        for (auto c : visible_clients) {
            if (c->is_floating) {
...
void SvarkaWM::Manage(Window w, XWindowAttributes* wa) {
        ...
        if (XGetWindowProperty(display, w, netatoms[NetWMWindowType], 0, 8L, False, XA_ATOM, &actual_type, &actual_format, &nitems, &bytes_after, &prop) == Success && prop) {
            ...
            for (unsigned long i = 0; i < nitems; i++) {
                if (types[i] == netatoms[NetWMWindowTypeDialog]) c->is_floating = true;
                if (types[i] == netatoms[NetWMWindowTypeUtility] || types[i] == netatoms[NetWMWindowTypeNotification]) {
                    c->is_floating = true;
                }
            }
            XFree(prop);
        ...
        } else if (c->workspace == current_workspace) {
            SetFocus(c);
        }
        Arrange();
    }
        ...
        if (it != clients.end()) {
            ...
            if (focused_client == c) {
                ...
                for (const auto& cl : clients) {
                    if (cl.get() != c && cl->workspace == current_workspace) { next_focus = cl.get(); break; }
                }
                SetFocus(next_focus);
            }
            clients.erase(it);
...
void SvarkaWM::Run() {
        ...
        SetupBar();
        XSelectInput(display, root, SubstructureRedirectMask | SubstructureNotifyMask | KeyPressMask | ButtonPressMask | ButtonReleaseMask);
            ...
            while (XEventsQueued(display, QueuedAfterReading) > 0) {
                ...
                if (ev.type == KeyPress) {
                    ...
                    if (it != clients.end()) {
                        ...
                        Arrange(); // Пересчитываем макет при изменении свойств окна
                    }
